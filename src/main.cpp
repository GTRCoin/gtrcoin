...
#if defined(NDEBUG)
# error "GTRcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x366bf4ae8bde708bdc8ac0f9ec0b7facc80759cb037ed7cd50f51c5cf46ad944");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // GTRcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "GTRcoin Signed Message:\n";
...
{
    // GTRcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 2000000); // GTRcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 24 * 60 * 60; // GTRcoin: 5 minutes
static const int64 nTargetSpacing = 2 * 60; // GTRcoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // GTRcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // GTRcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is GTRcoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xe9d251acd86aed7f03313c4d6be5f238bdbf780b26a38934601bc95cd7f8dc0a"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // GTRcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GTRcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GTRcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GTRcoinMiner(CWallet *pwallet)
{
    printf("GTRcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("gtrcoin-miner");
    ...
    try { loop {
        ...
        printf("Running GTRcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GTRcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GTRcoinMiner, pwallet));
}
